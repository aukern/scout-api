...
async def test_create_collection_returns_201(mock_client: AsyncClient) -> None:
    ...
    with patch("scout_api.collections.router.CollectionRepository") as MockRepo:
        ...
        response = await mock_client.post("/collections", json={"name": "my-research"})
...
async def test_create_collection_returns_409_on_duplicate(
    ...
    with patch("scout_api.collections.router.CollectionRepository") as MockRepo:
        instance = MockRepo.return_value
        instance.create = AsyncMock(side_effect=CollectionAlreadyExistsError("duplicate"))

        response = await mock_client.post("/collections", json={"name": "duplicate"})
...
async def test_create_collection_validates_name_characters(
    ...
    response = await mock_client.post("/collections", json={"name": "bad name!"})
...
async def test_response_uses_glossary_vocabulary(mock_client: AsyncClient) -> None:
    ...
    with patch("scout_api.collections.router.CollectionRepository") as MockRepo:
        instance = MockRepo.return_value
        instance.list_all = AsyncMock(return_value=[CollectionRow(id=1, name="alpha")])
...
async def test_full_crud_lifecycle(async_client: AsyncClient) -> None:
    ...
    # Create
    create_resp = await async_client.post("/collections", json={"name": "e2e-test"})
